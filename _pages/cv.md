---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

## Education
**Specialized Honors B.Sc. in Physics**  
York University - *Expected April 2026*

---

## Research Experience

### Quantum Optics - University of Toronto  
**Optical Box Trap for Ultracold Quantum Gases**  
*Apr 2025 – Sep 2025*

- Designed and implemented a fully functional optical box trap for ultracold atomic gases, including planning, simulation, optical design, testing, and experimental realization.
- Replaced harmonic trapping potentials, which produce inhomogeneous atomic densities, with a box-like optical potential, yielding a spatially homogeneous atomic cloud.
- Implemented sharply sculpted, blue-detuned optical potentials to confine a 3D cloud of 40K atoms.
- Applied advanced knowledge of atomic physics, statistical mechanics, Fourier optics, and atom trapping techniques.
- Developed and aligned complex optical systems involving fiber coupling, beam shaping, acousto-optics, optomechanics, and imaging.
- Performed computational simulations and data analysis to validate experimental and theoretical results.
- Collaborated within a large research group, managing milestones and coordinating experimental development.

---

### Experimental Atomic Physics - York University  
**Laser Intensity Stabilization Feedback System**  
*Apr 2024 – Sep 2024*

- Designed and constructed an analog feedback system for laser intensity stabilization, including a modular extension for pulsed waveform applications.
- Stabilized trap fluorescence and atom loading in a Magneto-Optical Trap (MOT), improving measurement reliability for precision metrology experiments.
- Implemented correction signals via RF control of an acousto-optic modulator (AOM) to suppress environmental perturbations.
- Applied expertise in laser systems, optical instrumentation, RF and analog electronics, signal processing, and experimental data analysis.
- Presented results at the **2024 NSERC USRA Conference** and **2024 Canadian Undergraduate Physics Conference (CUPC)**.
- Awarded **Top 8 Student Researcher** at CUPC 2024.

---

### Experimental Atomic Physics - York University  
**AOM-Modulated Frequency-Stabilized Laser Lock-In**  
*2024*

- Initiated construction of an AOM-modulated saturated absorption spectroscopy setup for laser frequency stabilization to rubidium hyperfine transitions.
- Designed and implemented optical layouts, RF electronics, and analog control circuitry.
- Achieved laser resonance and began development of a closed-loop frequency lock.
- Applied strong knowledge of optics, laser physics, RF systems, and signal processing.

---

## Technical Skills

- **Quantum & Atomic Physics:** Ultracold atoms, quantum optics, statistical mechanics  
- **Experimental Techniques:** Laser systems, optical trapping, beam shaping, imaging, optomechanics  
- **Electronics:** RF systems, analog circuits, AOMs, feedback control  
- **Programming & Data Analysis:** Python, numerical simulation, statistical analysis, signal processing  
- **Instrumentation & Fabrication:** Optical design, fiber coupling, laboratory instrumentation  

---

## Certifications & Training

- **Laser Safety Training Certification** - York University / University of Toronto (2024–2025)  
- **Machine Shop Training** - University of Toronto (June 2025)

---

## Awards & Honors

- **W.J. Megaw Prize in Experimental Physics** (2024)  
  *Highest grade in third-year experimental laboratory course (SC/PHYS 3220).*

- **NSERC USRA Research Award** - University of Toronto (2025)  
- **NSERC USRA Research Award** - York University (2024)

- **Allan D. & Josie A. Stauffer Scholarship in Theoretical Physics** (2024)  
  *Highest combined average in PHYS 3090 (Methods in Theoretical Physics) and PHYS 3040 (Quantum Mechanics).*

- **Top 8 Student Researcher**, Canadian Undergraduate Physics Conference (2024)

- **York University Renewable Entrance Scholarship** ($10,000)  
- **Faculty of Science Entrance Scholarship** ($2,000)

---

## Teaching & Outreach

**Private University Physics Tutor**  
*Jan 2025 – Apr 2025*  
- Delivered one-on-one tutoring in undergraduate physics, emphasizing problem-solving strategies and conceptual understanding.

**Math & Science Tutor - Mathnasium**  
*May 2023 – Nov 2024*  
- Tutored high-school mathematics and physics; prepared students for SAT-level material and advanced coursework.
